...
{
	if (!args[1])
		er(obj->data, "error: parse_em: lacks arg", NULL);
	if (!IS_DIGIT(args[1][0]))
		er(obj->data, "error: parse_em: arg not num", args[1]);
	obj->material.emision = parse_atof(obj->data, args[1], 0);
	if (obj->material.emision > 1 || obj->material.emision < 0)
		er(obj->data, "error: parse_em: emision 0-1", args[1]);
	if (args[2] && args[2][0] != '\n')
		er(obj->data, "error: parse_em: arg not valid", args[2]);
...
{
	if (obj->type != PL)
		er(obj->data, "error: parse_cb: cb just in PL", NULL);
	else if (!args[1] || !args[2])
		er(obj->data, "error: parse_cb: lacks arg", NULL);
	if (obj->type != PL)
		er(obj->data, "error: parse_cb: just in pl", args[1]);
	obj->material.board_scale = 1 - parse_atof(obj->data, args[1], 0);
	if (obj->material.board_scale > 0.991 || \
			obj->material.board_scale < 0.001)
		er(obj->data, "error: parse_cb: board size 0.991-0.001", NULL);
	...
	if (obj->material.m_type == MAT_DEFAULT && args[3][0] != '\n')
		er(obj->data, "error: parse_cb: arg not valid after cb", NULL);
	if (args[3] && args[4])
		er(obj->data, "error: parse_cb: arg not valid after cb", NULL);
}
...
void	parse_bm(t_obj *obj, char **args, int i)
{
	...
	if (obj->type != SP)
		er(obj->data, "error: parse_bm: bm just in SP", NULL);
	else if (!args[i] || !args[i + 1])
		er(obj->data, "error: parse_bm: lacks arg", NULL);
	obj->material.bm_size = (unsigned int)parse_atoi(obj->data, args[i], 0, 0);
	if (obj->material.m_type != -1 && (!args[i] || !args[i + 1]))
		er(obj->data, "error: parse_bm: arg not valid", NULL);
	...
	if (obj->material.m_type == TX)
		...
		er(obj->data, "error: parse_bm: arg not valid after bm", NULL);
	if (obj->material.m_type == MAT_DEFAULT && args[3][0] != '\n')
		er(obj->data, "error: parse_bm: arg not valid after bm", NULL);
	free(tmp);
}
...
void	parse_tx(t_obj *obj, char **args, int i)
{
	...
	if (obj->material.m_type != -1 && (!args[i] || !args[i + 1]))
		er(obj->data, "error: parse_tx: arg not valid", NULL);
	tmp = str_trim(args[i + 1], " \n\t");
	obj->material.texture = mlx_load_png(tmp);
	if (!obj->material.texture)
		er(obj->data, "error: parse_tx: bm not valid", NULL);
	if (args[i + 2])
		obj->material.m_type = type_extra_func(args[i + 2]);
	if (obj->material.m_type == BM)
		parse_tx_and_bm(obj, args, tmp, i);
	else if (args[i + 2] && args[i + 3])
		er(obj->data, "error: parse_tx: arg not valid after tx", NULL);
	if (obj->material.m_type == MAT_DEFAULT && args[3][0] != '\n')
		er(obj->data, "error: parse_tx: arg not valid after tx", NULL);
	free(tmp);
}
...
void	extra_functionalities(t_obj *obj, char *tmp)
{
	...
	if (obj->material.m_type == GL && obj->type == GL)
		er(obj->data, "error: extra_functs: GL just in SP", NULL);
	...
	if (obj->material.m_type == MAT_DEFAULT || !str[1] || \
		...
		er(obj->data, "error: extra_functs: char after color", str);
