...
void	create_alight(t_data *data, char *str, int type)
{
	...
	tmp = ft_substr(str, (unsigned int)skip_double(data, str, 1, 0), ft_strlen(str));
...
void	cam_blur(t_data *data, char *str2, char *tmp)
{
	...
	if (tmp[i] && ft_isdigit(tmp[i]))
	{
		...
		str2 = ft_substr(tmp, (unsigned int)skip_double(data, tmp, 0, 0), ft_strlen(tmp));
...
void	create_cam(t_data *data, char *str, int type)
{
	...
	tmp = ft_substr(str, (unsigned int)skip_doubles(data, str, 0, 0), ft_strlen(str));
	data->cam->axis = normalize(doubles_acl_parse(data, tmp, 0));
	str2 = ft_substr(tmp, (unsigned int)skip_doubles(data, tmp, -1, 0), ft_strlen(tmp));
	data->cam->fov = ft_atoi_parse(data, str2, 0, 1);
	free(tmp);
	tmp = ft_substr(str2, (unsigned int)skip_double(data, str2, 0, 0), ft_strlen(str2));
...
void	create_slight(t_data *data, t_slight **s_light, char *str, int type)
{
	...
	tmp[0] = ft_substr(str, (unsigned int)skip_doubles(data, str, 0, 0), ft_strlen(str));
	new_light->br = ft_atof(data, tmp[0], 0);
	tmp[1] = ft_substr(tmp[0], (unsigned int)skip_double(data, tmp[0], 0, 0), \
											ft_strlen(tmp[0]));
